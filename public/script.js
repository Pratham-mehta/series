...
class SearchModal {
    ...
    async openIMessage() {
        ...
        // Save profile to backend before opening iMessage
        await this.saveProfileToBackend(userName);
    ...
    async saveProfileToBackend(userName) {
        try {
            // Parse profile data from selectedProfile
            const profile = {
                name: userName,
                title: this.extractTitle(this.selectedProfile.title),
                company: this.extractCompany(this.selectedProfile.title),
                linkedinUrl: this.selectedProfile.link || '',
                skills: []
            };
            ...
            if (!userPhone) {
                console.log('User cancelled phone number input');
                return;
            }

            console.log('Saving profile to backend...', { phoneNumber: userPhone, profile });
    ...
    extractTitle(fullTitle) {
        if (!fullTitle) return '';
        // Extract job title (usually after " - ")
        const parts = fullTitle.split(' - ');
        if (parts.length > 1) {
            return parts[1].split(' at ')[0].split(' | ')[0].trim();
        }
        return '';
    }

    extractCompany(fullTitle) {
        if (!fullTitle) return '';
        // Extract company (usually after " at ")
        if (fullTitle.includes(' at ')) {
            return fullTitle.split(' at ')[1].split(' | ')[0].trim();
        }
        return '';
    }
